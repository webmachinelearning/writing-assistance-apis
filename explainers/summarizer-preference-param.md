...
## Problem Description
The current Summarizer API largely relies on powerful, high-parameter count on-device models. While these models offer high-quality outputs and a broad feature set, can imply significant "pain points" for web adoption:
...
*   **`"capability"`:** The implementation should prioritize the comprehensiveness and coherence of the summarization. This approach focuses on accurately capturing subtle context and producing highly refined summaries, which may result in higher latency and slower execution speeds.
...
*   A real-time grammar and spell checker using the Rewriter API would prioritize `"speed"` to ensure a smooth typing experience without blocking the user interface.
